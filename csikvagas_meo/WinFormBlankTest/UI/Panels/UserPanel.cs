...
namespace WinFormBlankTest
{
    public partial class UserPanel : Panel
    {
        ...
        delegate void SetBarCode2TextToDelegate(string text_in);
        public void SetBarCode2TextTo(string text_in)
        {
            Trace.TraceWarning("SetBarCode2TextTo {0}; Name: {1}",text_in,Name);
            if (tbBarcode2.InvokeRequired)
            {
                tbBarcode2.Invoke(new SetBarCode2TextToDelegate(SetBarCode2TextTo), text_in);
            }
            else
            {
                Trace.TraceWarning("Changing tbBarCode2.Text To {0}; and set it enabled=false; Name: {1}", text_in, Name);
                tbBarcode2.Text = text_in;
